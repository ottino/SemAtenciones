...
<TITLE>L_Estachofer.php</TITLE>
</HEAD>
<BODY>

<?
echo titulo_encabezado ('Listado Estadistico por Choferes' , $path_imagen_logo);
$segmenu = valida_menu();
if ($segmenu <> "OK")
...
$result=mysql_query("SELECT * FROM legajos where funcion = 3 order by 2");
$chofer= '<select name="cla_chofer" style="background-color:'.$se_color.'"><option value="0">Chofer</option>';

while ($row=mysql_fetch_array($result))
{
$chofer.='<option value="'.$row['legajo'].'">'.$row['apeynomb'].'</option>';
}
...
ACTION="L_Estachofer1.php">

<table width = "10%" align="left">
<TR>
<TD><? echo $chofer ?></td></tr>
...
<?
 $vengo = "CHOFER";
 echo '<input size= 12 type = "hidden" name = "cla_vengo" value="'.$vengo.'"/>';
?>
