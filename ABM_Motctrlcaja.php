...
<?
echo titulo_encabezado ('ABM de Motivos Mov Caja' , $path_imagen_logo);
$segmenu = valida_menu();
if ($segmenu <> "OK")
...
$result=mysql_query("select * from cjmotivos order by 2,1");


echo '</p><table ><tr><td>';
echo '<FORM METHOD="POST" NAME="formulario3" ACTION="A_Motctrlcaja.php">';
...
echo '<tr><td>'.$row["cjmotcodigo"].'</td>';

$motivos = $row["cjmotid"];

echo '<td>'.$row["cjmotdesc"].'&nbsp;</td>';

echo '<FORM METHOD="POST" NAME="formulario2" ACTION="B_Motctrlcaja.php">';
...
echo '<FORM METHOD="POST" NAME="formulario2" ACTION="M_Motctrlcaja.php">';
echo '<input type="hidden" name= "pasmotivo" value="'.$motivos.'" >';
echo ' <td width="17" align="center" style="background-color:'.$td_color.'">
                    <label onclick="this.form.submit();" style="CURSOR: pointer" >
