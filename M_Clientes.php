...
<HTML>
<HEAD>
<TITLE>M_Clientes.php</TITLE>
</HEAD>
<script>
 function valida (cli,nombre,perfis,perju,tipodni,tipolc,tipole,tipodne,dni,afil,dia,mes,anio,calle,nro,postal,cuit,sexo_m,sexo_f,
                                                                                     ...
                                                                                     }else if ((postal == null) || (postal == ''))
                                                                                     {
                                                                                      alert("Ingrese un numero postal valido")
                                                                                     }else if (perfis==true &&((sexo_m==false) || (sexo_f==false)))
                                                                                            {
                                                                                             ...
                                                                                             bandera_1 = 1;
                                                                                             //window.location.href("M_clientes2.php");
                                                                                            }
    ...
    if ((bandera_1 == 1) && (bandera_2 == 1))
      {
       ...
       location.href="M_clientes2.php?cli="+cli+"&nom="+nombre+"&perfis="+perfis+"&perju="+perju+"&tipodni="+
...
echo titulo_encabezado ('Modificación de Clientes' , $path_imagen_logo);
//echo "AAAAAAAAAAAAAAAAAA".$_POST['pasacli'];
/*
$segmenu = valida_menu();
if ($segmenu <> "OK")
...
ACTION="M_Clientes2.php">

<?
// id del cliente a editar
$cliente_editar =$_POST['pasacli'];
$cla_cli =$_POST['pasacli'];

$consulta_datos_cliente=mysql_query("SELECT *
                                     FROM clientes
                                     WHERE idcliente =".$cliente_editar);

$vector_datos_cli= mysql_fetch_array($consulta_datos_cliente);
...
{
if ($vector_datos_cli['provincia'] == $row['idprovincia'])
{
 $provincias.='<option selected="selected" value="'.$row['idprovincia'].'">'.$row['provincia'].'</option>';
}else {
...
while ($row=mysql_fetch_array($result))
{
if ($vector_datos_cli['localidad'] == $row['idlocalidad'])
{
  $localidad.='<option selected="selected" value="'.$row['idlocalidad'].'">'.$row['localidad'].'</option>';
}else
     {
...
while ($row=mysql_fetch_array($result))
{
if ($vector_datos_cli['estadocivil'] == $row['idestado'])
{
  $estadocivil.='<option selected="selected" value="'.$row['idestado'].'">'.$row['estado'].'</option>';
}else {
...
while ($row=mysql_fetch_array($result))
{
if ($vector_datos_cli['idzona'] == $row['idzonas'])
{
   $zonas.='<option selected="selected" value="'.$row['idzonas'].'">'.$row['desczonas'].'</option>';
}else {
...
switch (strtoupper ($vector_datos_cli['sexo'])) {
    case 'F':
        $sexo_f= 'checked="checked"';
        $sexo_m='';
        break;
    case 'M':
        $sexo_f='';
...
switch ($vector_datos_cli['tipoper']) {
    case 'F':
        $tipo_per_f = 'checked="checked"';
        $tipo_per_j = '';
        break;
    case 'FI':
        $tipo_per_f = 'checked="checked"';
...
$fnac=explode("-",$vector_datos_cli['fnacimiento']);
$dia  = $fnac[2];
$mes  = $fnac[1];
$anio = $fnac[0];
 echo '<Table>';
 echo '<TR><TD>Nombre   </TD><TD><input size= 50 type = "text" name = "cla_nombre" value="'.$vector_datos_cli['nombre'].'" /></TD>';
 echo '<TD>Tipo Pers</TD><TD>
     <label>
       <input type="radio" name="cla_tipoper_fis" value="F"   '.$tipo_per_f.'
                                                                                      onclick="document.formulario.cla_tipoper_ju.checked=false"/>
       P.Fisica</label>
     <label>
       <input type="radio" name="cla_tipoper_ju" value="J" '.$tipo_per_j.'

                                                            onclick="document.formulario.cla_tipoper_fis.checked=false
 ...
 echo '<TR><TD>Cuit     </TD><TD><input size= 30 type = "text" name = "cla_cuit" value="'.$vector_datos_cli['cuit'].'" /></TD>';

 echo '<TD>Sexo</TD><TD>
     <label>
       <input type="radio" name="cla_sexo_m" value="M" '.$sexo_m.' onclick=" document.formulario.cla_sexo_f.checked=false"/>
       Masculino</label>
     <label>
       <input type="radio" name="cla_sexo_f" value="F" '.$sexo_f.' onclick=" document.formulario.cla_sexo_m.checked=false"/>
...
echo '<TD>Nro Docum</TD><TD><input size= 10 type = "text" name = "cla_documento" value="'.$vector_datos_cli['documento'].'" /></TD></TR>';
echo '<TR><TD>Nro Afil.</TD><TD><input size= 10 type = "text" name = "cla_nroafiliado" value="'.$vector_datos_cli['nroafiliado'].'" /></TD></TR>';
echo '<TR><TD>Est.Civil</TD><TD><select name="cla_estadocivil">'.$estadocivil.'</select></TD>';
echo '<TD>Fec.Nac. </TD><TD><input size= 2 type = "text" name = "cla_fdnac" value="'.$dia.'"/>
                            <input size= 2 type = "text" name = "cla_fmnac" value="'.$mes.'"/>
                            <input size= 4 type = "text" name = "cla_fanac" value="'.$anio.'"/>
                   </TD></TR>';
echo '<TR><TD>Calle    </TD><TD><input size= 50 type = "text" name = "cla_calle" value="'.$vector_datos_cli['calle'].'" /></TD>';
echo '<TD>Nro      </TD><TD><input size= 4 type = "text" name = "cla_nrocalle"   value="'.$vector_datos_cli['nrocalle'].'" /></TD></TR>';
echo '<TR><TD>Entre   </TD><TD><input size= 50 type = "text" name = "cla_entre1" value="'.$vector_datos_cli['entre1'].'" /></TD>';
echo '<TD>Y</TD><TD><input size= 50 type = "text" name = "cla_entre2"            value="'.$vector_datos_cli['entre2'].'" /></TD></TR>';
echo '<TR><TD>Piso     </TD><TD><input size= 4 type = "text" name = "cla_piso"   value="'.$vector_datos_cli['piso'].'" /></TD>';
echo '<TD>Depto    </TD><TD><input size= 6 type = "text" name = "cla_depto"      value="'.$vector_datos_cli['depto'].'" /></TD></TR>';
echo '<TR><TD>Barrio   </TD><TD><input size= 30 type = "text" name ="cla_barrio" value="'.$vector_datos_cli['barrio'].'" /></TD>';
echo '<TD>Cod.Post.</TD><TD><input size= 4 type = "text" name = "cla_cpostal"    value="'.$vector_datos_cli['cpostal'].'" /></TD></TR>';
...
echo '<TR><TD>Telefono </TD><TD><input size= 20 type = "text" name = "cla_telefono" value="'.$vector_datos_cli['telefono'].'" /></TD>';
echo '<TD>Celular  </TD><TD><input size= 20 type = "text" name = "cla_celular"      value="'.$vector_datos_cli['celular'].'"/></TD></TR>';
echo '<TR><TD>E-mail   </TD><TD><input size= 40 type = "text" name = "cla_email"    value="'.$vector_datos_cli['email'].'" /></TD></TR>';
echo '<TR><TD>Contacto </TD><TD><input size= 50 type = "text" name = "cla_contacto" value="'.$vector_datos_cli['contacto'].'" /></TD></TR>';
echo '<TR><TD>Observaciones</TD><TD><textarea name="cla_observaciones" rows="5" cols="40">'.$vector_datos_cli['observaciones'].'</textarea></TD></TR>';
echo '<TR><TD><input type ="hidden" name = "cla_cli" value="'.$cla_cli.'" /></TD></TR>';
echo '</table>' ;
echo '<input type="hidden" value="">'
?>
<td><INPUT TYPE="button" value="Modificar" onclick="
                                                   valida (document.formulario.cla_cli.value ,
                                                           document.formulario.cla_nombre.value ,
