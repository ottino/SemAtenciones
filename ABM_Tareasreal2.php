...
<HTML>
<HEAD>
<TITLE>ABM_Tareasreal2.php</TITLE>
</HEAD>
...
<?
echo titulo_encabezado ('Mantenimiento Realizado' , $path_imagen_logo);
$segmenu = valida_menu();
if ($segmenu <> "OK")
...
$movil = $_POST["movil"];
 ...
 $result= mysql_query("select * from tareareal a, moviles b, tareas c, proveedores d where
         a.idmovil = b.idmovil and a.idtarea = c.idtarea and a.idproveedor = d.idproveedores and a.idmovil = '".$movil."' order by 1 ");

echo '</p><table ><tr><td>';
echo '<FORM METHOD="POST" NAME="formulario3" ACTION="A_Tareasreal.php">';
...
echo ' <td><td align="center" style="background-color:'.$td_color.'">
                    <label onclick="this.form.submit();" style="CURSOR: pointer" >
                     <img align="middle" alt=\'Nuevo Mantenimiento Pendiente\' src="imagenes/MANTREAL.ICO" width="50" height="40"/>
                    </label>
                  </td></td>';
echo '</FORM>';
echo'</table></table>';
...
echo '<FORM METHOD="POST" NAME="formulario2" ACTION="B_Tareasreal.php">';
echo '<input type="hidden" name= "pasatareareal" value="'.$idnovedad.'" >';
...
echo '<FORM METHOD="POST" NAME="formulario2" ACTION="M_Tareasreal.php">';
echo '<input type="hidden" name= "pasatareareal" value="'.$idnovedad.'" >';
echo ' <td width="17" align="center" style="background-color:'.$td_color.'">
                    <label onclick="this.form.submit();" style="CURSOR: pointer" >
